...
BybitSpotExchangeConnector::BybitSpotExchangeConnector() : m_p(std::make_unique<P>()) {
    m_p->restClient = std::make_unique<bybit::RESTClient>("","");
}

BybitSpotExchangeConnector::~BybitSpotExchangeConnector() {
    m_p->restClient.reset();
}

std::string BybitSpotExchangeConnector::exchangeId() const {
    return std::string(magic_enum::enum_name(ExchangeId::BybitSpot));
}

std::string BybitSpotExchangeConnector::version() const {
    return "1.0.4";
}
...
std::int64_t BybitSpotExchangeConnector::getServerTime() const {
